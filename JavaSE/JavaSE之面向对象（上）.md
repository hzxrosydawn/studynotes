...
static关键字用于修饰成员变量、方法、内部类和内部接口等，表示这些成员属于类本身，而不是属于该类的单个实例。因此，static修饰的成员变量、方法也称为类变量（也称静态变量）、类方法（也称静态方法）。

不使用static修饰的普通成员变量、普通方法则属于该类的单个实例，而不是该类。通常把不使用static修饰的成员变量、方法称为实例变量（也称非静态变量）、实例方法（也称非静态方法）。

**使用static修饰的方法或字段既可以使用类名作为调用者，也可以使用实例名作为调用者（这种方式底层还是使用类名作为调用者的。不过不推荐使用这种方式，易混淆）。建议使用类名调用static修饰的成员，这样语义更清晰。使用实例名调用static修饰的方法或字段产生的影响对该类的其他实例同样有效，因为这些实例共享同一个类**。而没有使用static修饰的方法或字段只能使用实例名作为调用者。

**static修饰的方法中不能直接调用例方法或实例字段，而实例方法中可以直接调用static修饰的方法或字段**。
...
**父类中private修饰的私有成员对其子类是完全隐藏的，子类无法直接访问父类中的私有成员，只能通过调用从父类继承的方法来调用（或改变）父类的私有成员（在子类中也不能通过父类对象来引用父类的private成员）。所以，即使子类中定义了一个与父类中私有成员（成员变量或方法）同名（修饰符可以不同）的成员，也不是重写，而是一个新的定义**。
